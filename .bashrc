...
## PS1 CONFIG {{{
  ...
  if $_isxrunning; then
    ...
    export PS1="$GY$Y\u$GY\[\033[0m\033[0;32m\]@$P\h$GY:$B\W$LB\$(get_git_info)$GY
...
## ALIAS {{{
  ...
  # MODIFIED COMMANDS {{{
    ...
    alias nano='nano -w'
    alias ping='ping -c 2'
    alias smonta='sudo umount /dev/sdc'
    alias performance='sudo cpupower frequency-set -g performance'
    alias ondemand='sudo cpupower frequency-set -g ondemand'
    alias powersave='sudo cpupower frequency-set -g powersave'
    alias optirun='VGL_READBACK=pbo optirun glxspheres'
    alias primusrun='vblank_mode=0 primusrun'
    alias update-grub='sudo grub-mkconfig -o /boot/grub/grub.cfg'
    alias tempo='systemd-analyze time && systemd-analyze blame'
    alias rimuovi-orfani='sudo pacman -Rsn $(pacman -Qqdt)'
    alias ram-free='watch -n 1 cat /proc/meminfo'
    alias svuota='sudo sysctl -w vm.drop_caches=3'
    alias updb='sudo updatedb'
    alias monta='sudo mount -a'
    alias x='startx'
  #}}}
  # PRIVILEGED ACCESS {{{
    if ! $_isroot; then
      ...
      alias dhroot='sudo dolphin'
      alias throot='sudo thunar'
      alias fstab='sudo nano /etc/fstab'
      alias swapon='sudo swapon /dev/sdb1'
      alias swapoff='sudo swapoff /dev/sdb1'
      alias compilazioni='sudo nano /etc/yaourtrc'
    fi
  #}}}
  # PACMAN ALIASES {{{
    # we're on ARCH
...
## FUNCTIONS {{{
  ...
  # CONVERTI A ISO {{{
    to_iso () {
      ...
      for i in $*; do
        if [[ ! -f "$i" ]]; then
          echo "'$i' non è un file valido,lo salto"
        else
          echo -n "converting $i..."
  ...
  #}}}
#}}}
[ ! "$UID" = "0" ] &&  alsi -l --bold=red && echo -e $LRED "                   Benvenuto in $(alsi | awk -F ':\033\\\[0m ' '/OS/ {print $2}' 2>/dev/null | tr -d '\n') su $HOSTNAME di $LOGNAME $Z"
